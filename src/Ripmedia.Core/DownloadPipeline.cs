...
public sealed class DownloadPipeline(YtDlpClient ytDlp, MediaTagger tagger)
{
    public async Task<DownloadResult> RunAsync(DownloadRequest request, RipmediaSettings settings, Action<StageResult>? stage, Action<DownloadProgress>? progress, CancellationToken cancellationToken)
    {
        ...
        var format = FormatRules.Normalize(request.Format);
        if (format is not null && !FormatRules.IsSupported(format)) throw new ArgumentException($"Unsupported format '{format}'.");
        ...
        {
            MediaItem root;
            try { root = await ytDlp.InspectAsync(url, request.CookieFile ?? settings.CookieFile, request.BrowserCookies ?? settings.BrowserCookieProfile?.YtDlpValue, request.NoPlaylist, cancellationToken); }
            ...
            foreach (var item in entries)
            {
                ...
                try
                {
                    ...
                    var media = await ytDlp.DownloadAsync(item, temp, request with { Format = format, CookieFile = request.CookieFile ?? settings.CookieFile, BrowserCookies = request.BrowserCookies ?? settings.BrowserCookieProfile?.YtDlpValue }, progress, cancellationToken);
